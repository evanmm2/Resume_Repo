...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    while(val > elements[i]){            //While the value is greater than the other elements, increases element number. Once the element is larger, then return that location -1
      if(i == elements.size()-1){           // If index location is the last possible, then exit with index location +1
