...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
     ...
     * are looking for, we are done. */
       ...
       if(subroot->elements.size() != first_larger_idx && subroot->elements.empty() != true){
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    parent->elements.insert(elem_itr, child->elements[mid_elem_idx]);         //Adds the middle of the child to the parent.
    parent->children.insert(child_itr, new_right);                            //Adds the new node we've made to the children

    // keep in mind that assign can take in iterators and populates the vector with the first until the second inputs. Does input the first iterator it starts on. Does not input the last
    new_right->elements.assign(mid_elem_itr + 1, child->elements.end());      // Makes the right one have the right half(accounting for the balance so that it is accepted in our test) of elements
    new_right->children.assign(mid_child_itr, child->children.end());         // Makes the children

    new_left->elements.assign(child->elements.begin(), mid_elem_itr);         // Does with the left what was done to the right. Keep in mind mid_elem_itr, is not included because it is considered the end of our iterator for this purpose
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */
    if(subroot->elements.size() != first_larger_idx && subroot->elements.empty() != true){          // If the subroot is already there, exit
      if(subroot->elements[first_larger_idx] == pair){
       return;
       }
     }
     if (subroot->is_leaf) {                                                            // If the subroot we are looking at is in the leaf, then input it there according to the beginning of the elements vector
     subroot->elements.insert(subroot->elements.begin() + first_larger_idx, pair);
        }
     else {
     insert(subroot->children[first_larger_idx], pair);                               // If it is not in a leaf node, check the order and, if necessary, if it should be split
