...
class InorderTraversal : public TreeTraversal<T> {
  public:
    ...
    void add(typename BinaryTree<T>::Node *& treeNode) {
      ...
      typename BinaryTree<T>::Node* temp = treeNode;

      if(stack.top()->left!=treeNode){
      if(stack2.top()!=treeNode){
  ...
  private:
    ...
    std::stack<typename BinaryTree<T>::Node*> stack2;
