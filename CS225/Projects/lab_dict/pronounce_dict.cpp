...
{
    /* Your code goes here! */
    string wordx = word1;   //copies em
    string wordy = word2;
    // uppercases out copies
    std::transform(wordx.begin(), wordx.end(), wordx.begin(), ::toupper);
    std::transform(wordy.begin(), wordy.end(), wordy.begin(), ::toupper);
    // Now we've gotta search the dictionaries and compare the strings
    map<string, vector<string>>::const_iterator lookupx = dict.find(wordx);
    map<string, vector<string>>::const_iterator lookupy = dict.find(wordy);

    if(lookupx == dict.end() || lookupy == dict.end()){
      return false; //if one of them isn't in the dictionary, then there is no homophone
    }
    if(lookupx->second == lookupy->second){
      return true;
    }
    return false;
}
