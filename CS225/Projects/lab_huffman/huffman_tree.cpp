...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     while((singleQueue.size() + mergeQueue.size()) != 1){
       TreeNode* Node0 = removeSmallest(singleQueue, mergeQueue);           //0
       TreeNode* Node1 = removeSmallest(singleQueue, mergeQueue);           //1

       TreeNode* Head = new TreeNode(Node0->freq.getFrequency() + Node1->freq.getFrequency());
       Head->left = Node0;
       Head->right = Node1;

       mergeQueue.push(Head);
     }
     root_ = mergeQueue.back();
