...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      double x_1 = x;
      double y_1 = y;
      double x_length = centerX-x_1;
      double y_length = centerY-y_1;
      double z_length = sqrt(x_length * x_length + y_length * y_length);
      if(z_length > 160){
        pixel.l = pixel.l*.20;
      }
      else{
        pixel.l = pixel.l - pixel.l*z_length*.005;
      }
    }}
