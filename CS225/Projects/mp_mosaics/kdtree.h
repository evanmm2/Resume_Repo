...
class KDTree
{
  ...
  private:
     ...
     */
     void construct(KDTreeNode* &subroot, int left, int right, int dimension);
     KDTreeNode* quickSelect(int left, int right, int midpoint, int dimension);
     int partition(int left, int right, int mid, int dimension);
     void recursiveHelper(double & distance, int elem,  KDTreeNode* node, KDTreeNode*& currentBest, Point<Dim> query) const;
     double getDistance(const Point<Dim>& point0, const Point<Dim> & point1) const;
   };
